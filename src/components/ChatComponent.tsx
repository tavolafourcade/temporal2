...
const ChatComponent: React.FC<ChatComponentInterface> = ({ docId }) => {
  const [selectedConversationId, setSelectedConversationId] = useState<
    string | null
  >(null);
  const [selectedOption, setSelectedOption] = useState<string>(docId);
  ...
  useEffect(() => {
    ...
    if (
      ...
      selectedOption == CONSTANTS.RAZ_ID ||
      selectedOption == CONSTANTS.AVOCADEMY_ID
    ) {
      setAiLead(true);
    } else {
      setAiLead(false);
  ...
  if (
    ...
    claim != "mlp" &&
    claim != "ivyhouse" &&
    claim != "socialsnowball" &&
    claim != "fancentro" &&
    claim != "solarceos" &&
    claim != "infinitybox" &&
    claim != "fastfunds" &&
    claim != "smarttradingblueprint" &&
    claim != undefined
  ) {
    return (
      <div>
