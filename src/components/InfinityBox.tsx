...
const idToAliasMapping = {
  Gfv4WyTTYUYD3uPLvmRm: "Infinity Box",

  // Add more mappings if necessary
};

const notify = () => toast.success("Group Chat Toggle Success");
const failed = () => toast.error("Group Chat Toggle Failed");
...
export default function InfinityBox() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      user.getIdTokenResult(true).then((tokenResult) => {
        ...
        const { isRazAdmin = false, account: userCompany } =
          tokenResult.claims || {};

        const shouldAllowAccess =
          isRazAdmin || (userCompany && `/${userCompany}` === currentHref);
  ...
  const toggleGroupChat = async (e: React.FormEvent) => {
    let tok = await user?.getIdToken(true);
    const result = await fetch(
      process.env.NEXT_PUBLIC_URL + "/api/get_api_key",
      {
    ...
    const res = await fetch(
      process.env.NEXT_PUBLIC_URL + "/api/start_group_chat",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      <div style={{ width: "30%" }}>
        ...
        */}
        ...
        <div className="flex flex-col w-36">
          ...
          {(user?.uid === CONSTANTS.HUNTER_ID ||
            user?.uid == CONSTANTS.CASEY_ID ||
            user?.uid == CONSTANTS.TYLER_ID ||
            user?.uid == CONSTANTS.BRAYAN_ID ||
            user?.uid == CONSTANTS.DALE_ID ||
            user?.uid == CONSTANTS.JESS_ID ||
            user?.uid == CONSTANTS.KIM_ID ||
            user?.uid == CONSTANTS.KIM_ID_2 ||
